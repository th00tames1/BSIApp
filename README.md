...
7. **설정 → 데이터** — **백업 내보내기/불러오기**. 기록·사진(카메라 원본 포함)·원시 번들·조사지·설정 전부를
...
촬영할 때마다 원시 번들(`documents/raw/{조사목}_{시각}/`)에 **카메라 원본을 바이트 그대로**(`{방위}_original.jpg`,
역광 재촬영 전 첫 사진은 `_original_backlit.jpg`) 남기고, `{방위}_capture.json`에 셔터·저장 시각(시간대·UTC·epoch ms),
배율·노출(하드웨어/소프트웨어 몫·밝기 배율과 변환식)·원본/표준 해상도·원본 EXIF(기기·셔터·ISO·초점거리)를 적는다.
