...
5. **결과** — BSI 심각도 미터 + 고사확률 게이지, 방위별 비율, 그을음 높이/폭, DBH 입력 시 재계산
...
```
minSdk 24 · compileSdk 35. 권한(카메라·위치·인터넷)은 매니페스트에 포함.
...
- **수고봉 검출이 휴리스틱**(밝은 노랑 수직 영역)이다. 논문 파이프라인의 YOLO **OBB 1 m 구간 모델**로
  교체하는 것이 남은 과제 — 연결 지점은 `PoleScale.detect`(`seg_decoder.dart`) 한 곳이며
  `analysis_service.dart`가 이를 소비한다. **수고봉이 검출되지 않으면 그 방위의 높이(m)를 못 구해
  BSI 합에서 빠진다**(BSI 자체가 비어질 수 있음)
- **고사확률 계수는 근사**(Kwon 오즈비 + 절편 피팅). 정확한 BSI×DBH 표를 받으면 `mortality.dart` 교체
- **DBH(추정)** 은 줄기 하부 폭 × (1/px_per_m)의 간이 추정. 결과 화면에서 실측값을 입력하면 판정이 재계산된다
