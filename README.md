# BSI_app — 산불피해목 진단·복원 지원 앱 (bsi_field)
...
3. **촬영** — 동/서/남/북 4방위. 실시간 나침반(진북/자북), 배율·노출 보정, GPS 상태 표시
   (빨강·주황·초록), 방위별 GPS 입지 태깅, 갤러리에서 기존 사진 불러오기
...
7. **설정 → 데이터** — **백업 내보내기/불러오기**. 기록·사진·원시 번들·조사지·설정 전부를
   `내장 저장공간/Download/BSI_backup/`의 ZIP으로 만들고(앱을 지워도 남음), 새 폰에서 합쳐 그대로 이어서 조사한다.
   3.5 GB를 넘으면 조각으로 나뉜다(ZIP32 한계). 이미 있는 기록·파일은 건너뛰므로 두 번 불러와도 안전
8. **설정 → 앱 → 앱 업데이트**(안드로이드) — GitHub 릴리스의 새 APK를 받아(끊기면 이어 받기) 설치 화면을 연다.
   릴리스는 버전 태그를 올리면 GitHub Actions가 한 키로 서명해 게시한다 — [SETUP.md 7장](SETUP.md)

촬영할 때마다 원시 번들(`documents/raw/{조사목}_{시각}/`)에 **분석용 표준 사진**(`{방위}_photo.jpg`, 긴 변 2560 —
그대로 모델에 다시 넣을 수 있다)과 `{방위}_capture.json`을 남긴다. json에는 셔터·저장 시각(시간대·UTC·epoch ms),
배율·노출(하드웨어/소프트웨어 몫·밝기 배율과 변환식)·카메라/표준 해상도·카메라 파일 EXIF(제조사·모델·셔터·ISO)를
적는다. 카메라 원본 파일은 저장이 오래 걸려 보관하지 않는다(EXIF는 파일 머리말만 읽는다).
